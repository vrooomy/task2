...
public class MyGame extends JPanel {
    ...
    public static void main(String[] args) {
        ...
        f.setSize(12 * 26 , 26 * 23 + 35);
        f.setVisible(true);

        final MyGame game = new MyGame();
        game.init();    //Initializing the game
        f.add(game);    //Appending the game to the frame

        f.addKeyListener(new KeyListener() {    //Add key listener in the panel
            public void keyTyped(KeyEvent e) {  
            }

            public void keyPressed(KeyEvent e) {    //Event detection
                switch (e.getKeyCode()) {   //Key value
                    case KeyEvent.VK_UP:
                        game.rotate(-1);
                        break;
    ...
    private final Color[] MyColors = {
            Color.cyan, Color.MAGENTA, Color.orange, Color.yellow, Color.black, Color.white,
            Color.red };
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 23; j++) {
                ...
                if(collidesAt(pt.x, 2, rotation))
            {
                g.setColor(Color.red);
                g.drawString("-----Game Over-----",10*12,15);
        ...
        g.drawString("Score : " + score, 11 * 12,45);
